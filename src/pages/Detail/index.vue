          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(attr, index) in spuSaleAttrList" :key="index">
                ...
                <!-- 给每一个属性绑定事件？  -->
                <dd changepirce="0" v-for="val in attr.spuSaleAttrValueList" :key="val.id"
                  @click="choose(val, attr.spuSaleAttrValueList)" :class="{ active: val.isChecked == 1 }">{{
                    val.saleAttrValueName }}</dd>
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <a @click="addCart(skuInfo.id, num)">加入购物车</a>
                <!-- 
                  1. 点加入购物车，路由跳转、传参
                  2. 调接口？？
                  3. 处理购物车页数据
...
export default {
  ...
  methods: {
    ...
    choose(val, valList) {
      // 排他思想，想让所有的isChecked为0，以下两种遍历数组方法均可
      // for(let v of valList){
      //    v.isChecked=0
      // }
      valList.forEach(v => { v.isChecked = 0 })
      val.isChecked = 1
    },
    changeNum() {
      // 用户输入的都是string！！！
  ...
  async  addCart(id, num) { 
   try{
    // 调接口
    await  this.$store.dispatch('addToCart',{skuId:id,skuNum:num})
    //  请求成功，跳转路由
      this.$router.push({
        path: '/addcartsuccess',
        query: {
